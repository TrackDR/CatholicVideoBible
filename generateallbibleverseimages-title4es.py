...
def genbibleverseimage(Book,Chapter,Verse,imgwidth,imgheight,numchars,imgtag,fontsize,gentxt,basedirectory):
    ...
    with open(filename2read, 'rt', encoding='utf-8-sig') as f:
    #with open(filename2read, 'rt', encoding='utf-8') as f:
        data = f.readlines()
    for line in data:
...
if __name__ == '__main__':
    ...
    for Chapter in range(startchp, startchp + numchps):
        ...
        filename2read = os.path.join(basedirectory, Book + "_" + Chapter_str + "aarm.txt")
        print(filename2read)
        ...
        # Find the last verse line (should be last line that starts with chapter#:verse#)
        for line in reversed(nonempty_lines):
            if ":" in line and "." in line:
                try:
                    chapter_verse = line.split(".")[0]  # e.g. "3:16"
                    verse_number = int(chapter_verse.split(":")[1])
                    print("Number of verses:", verse_number)
                    break
